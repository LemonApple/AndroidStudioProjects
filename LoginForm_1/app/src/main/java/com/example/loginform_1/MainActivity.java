package com.example.loginform_1;
...
import com.example.loginform_1.util.ViewUtil;
...
public class MainActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    private Switch sc_remember; // 声明一个开关对象

    private int mRequestCode = 0; // 跳转页面时的请求代码
    private int mType = 2; // 用户类型
    private String mPassword = "000000"; // 默认密码000000
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        sc_remember = findViewById(R.id.sc_remember);
    ...
    // 初始化用户类型的下拉框
    private String[] typeArray = {"樱花雨", "青花瓷", "邵然 18991941"};
    ...
    private class RadioListener implements RadioGroup.OnCheckedChangeListener {
        ...
        public void onCheckedChanged(RadioGroup group, int checkedId) {
            if (checkedId == R.id.rb_password) { // 选择了密码登录
                ...
                sc_remember.setVisibility(View.VISIBLE);
            } else if (checkedId == R.id.rb_verifycode) { // 选择了验证码登录
                tv_password.setText("　验证码：");
                et_password.setHint("请输入验证码");
                btn_forget.setText("获取验证码");
                sc_remember.setVisibility(View.INVISIBLE);
            }
        }
    }
    ...
    public void onClick(View v) {
        ...
        if (v.getId() == R.id.btn_forget) { // 点击了“忘记密码”按钮
            if (phone.length() < 11) { // 手机号码不足11位
                Toast.makeText(this, "您输入的位数不足，在中国，手机号码是11位的。", Toast.LENGTH_SHORT).show();
                return;
            }
            if (rb_password.isChecked()) { // 选择了密码方式校验，此时要跳到找回密码页面
                ...
                builder.setMessage("手机号" + phone + "，本次验证码是" + mVerifyCode + "，请填入验证码框中");
